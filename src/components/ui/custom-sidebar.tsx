...
type SidebarContext = {
  isMobile: boolean;
  getSidebarState: (key: string) => SidebarState;
  getSidebarStyles: (key: string) => SidebarStyles;
  setSidebarStyles: (key: string, styles: SidebarStyles) => void;
  setSidebarOpen: (key: string, open: boolean | ((open: boolean) => boolean)) => void;
  setSidebarOpenMobile: (key: string, open: boolean | ((open: boolean) => boolean)) => void;
  toggleSidebar: (key: string) => void;
};

const SidebarContext = React.createContext<SidebarContext | null>(null);

function useSidebar(sidebarKey: string) {
  const context = React.useContext(SidebarContext);
  if (!context) {
    throw new Error("useSidebar must be used within a SidebarProvider.");
  }

  const { isMobile, getSidebarState, getSidebarStyles, setSidebarStyles, setSidebarOpen, setSidebarOpenMobile, toggleSidebar: contextToggleSidebar } = context;

  const { open, openMobile, state } = getSidebarState(sidebarKey);
  ...
  return {
    ...
    state,
    isMobile,
    setOpen: (value: boolean | ((open: boolean) => boolean)) => setSidebarOpen(sidebarKey, value),
    setOpenMobile: (value: boolean | ((open: boolean) => boolean)) => setSidebarOpenMobile(sidebarKey, value),
    toggleSidebar: () => contextToggleSidebar(sidebarKey),
    getSidebarStyles: (sidebarKey: string) => getSidebarStyles(sidebarKey),
    setSidebarStyles: (sidebarKey: string, styles: SidebarStyles) => setSidebarStyles(sidebarKey, styles),
  };
}
...
>(({ defaultOpen = false, className, children, ...props }, ref) => {
  ...
  const [sidebars, setSidebars] = React.useState<Record<string, { open: boolean; openMobile: boolean }>>({});
  const [sidebarStylesMap, setSidebarStylesMap] = React.useState<Record<string, SidebarStyles>>({});
  ...
  //   (key: string, value: boolean | ((open: boolean) => boolean)) => {
  //     setSidebars((prev) => {
  //       const sidebarCount = Object.keys(prev).length;
  //       console.log(
  //         `Current number of sidebars: ${sidebarCount}. The open ones keys are `,
  //         Object.keys(prev).filter((key) => prev[key].open)
  //       );
  ...
  //         openMobile: false,
  //       };
  //       const newOpen = typeof value === "function" ? value(current.open) : value;

  //       // This sets the cookie to keep the sidebar state.
  //       document.cookie = `${SIDEBAR_COOKIE_NAME}:${key}=${newOpen}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`;

  //       return { ...prev, [key]: { ...current, open: newOpen } };
  //     });
  //   },
  //   [defaultOpen]
  // );
  ...
  const setSidebarOpen = React.useCallback(
    (key: string, value: boolean | ((open: boolean) => boolean)) => {
      setSidebars((prev) => {
        ...
        // If we're opening this sidebar
        if (newOpen && !current.open) {
          // Check if it's one of our special sidebars
          if (key === "workflowDashCanvasSidebar" || key === "workflowDashHistorySidebar") {
            // Get the other sidebar key
            const otherSidebarKey = key === "workflowDashCanvasSidebar" 
              ? "workflowDashHistorySidebar" 
              : "workflowDashCanvasSidebar";

            // Close the other sidebar if it's open
            if (prev[otherSidebarKey]?.open) {
              const result = {
                ...prev,
                [key]: { ...current, open: newOpen },
                [otherSidebarKey]: { ...prev[otherSidebarKey], open: false }
              };
              return result;
            }
          }
        ...
        // Return updated state for the current sidebar
        return {
          ...prev,
          [key]: { ...current, open: newOpen },
        };
  ...
  const setSidebarOpenMobile = React.useCallback(
    (key: string, value: boolean | ((open: boolean) => boolean)) => {
      setSidebars((prev) => {
        ...
        const newOpenMobile = typeof value === "function" ? value(current.openMobile) : value;
        return { ...prev, [key]: { ...current, openMobile: newOpenMobile } };
      });
    },
    [defaultOpen]
  );
  ...
  const toggleSidebar = React.useCallback(
    (key: string) => {
      if (isMobile) {
        setSidebarOpenMobile(key, (open) => !open);
      } else {
        setSidebarOpen(key, (open) => !open);
      }
    },
    [isMobile, setSidebarOpen, setSidebarOpenMobile]
  );

  const getSidebarStyles = React.useCallback(
    (key: string): SidebarStyles => {
      return (
        sidebarStylesMap[key] || {
          width: SIDEBAR_WIDTH,
  ...
  const setSidebarStyles = React.useCallback((key: string, styles: SidebarStyles) => {
    setSidebarStylesMap((prev) => ({
      ...prev,
      [key]: {
        ...prev[key],
        ...styles,
  ...
  React.useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === SIDEBAR_KEYBOARD_SHORTCUT && (event.metaKey || event.ctrlKey)) {
        ...
        // Specify which sidebar to toggle with keyboard shortcut if needed
        const defaultSidebarKey = "default";
        toggleSidebar(defaultSidebarKey);
      }
    };
  ...
  const contextValue = React.useMemo<SidebarContext>(
    ...
    [isMobile, getSidebarState, getSidebarStyles, setSidebarStyles, setSidebarOpen, setSidebarOpenMobile, toggleSidebar]
  );

  return (
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div className={cn("group/sidebar-wrapper flex min-h-svh w-full has-[[data-variant=inset]]:bg-sidebar", className)} ref={ref} {...props}>
          {children}
        </div>
      </TooltipProvider>
...
>(({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, sidebarKey, styles: propStyles, ...props }, ref) => {
  const { isMobile, state, openMobile, setOpenMobile, getSidebarStyles, setSidebarStyles } = useSidebar(sidebarKey);
  // Use a ref to track if we've initialized styles
  const stylesInitialized = React.useRef(false);

  // Only set styles once on mount if they exist
  React.useEffect(() => {
    if (propStyles && !stylesInitialized.current) {
      setSidebarStyles(sidebarKey, propStyles);
      stylesInitialized.current = true;
    }
  }, [sidebarKey, propStyles, setSidebarStyles]);

  const styles = getSidebarStyles(sidebarKey);

  console.log("The styles for ", sidebarKey, styles);
  ...
  if (collapsible === "none") {
    return (
      <div
        ...
        style={
          {
            "--sidebar-width": styles.width || SIDEBAR_WIDTH,
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          style={
            {
              "--sidebar-width": styles.width || SIDEBAR_WIDTH_MOBILE,
            } as React.CSSProperties
          }
          side={side}
        >
    ...
    <div ref={ref} className="group peer hidden md:block text-sidebar-foreground" data-state={state} data-collapsible={state === "collapsed" ? collapsible : ""} data-variant={variant} data-side={side}>
      ...
      <div
        ...
        style={
          {
            "--sidebar-width": styles.width || SIDEBAR_WIDTH,
            "--sidebar-width-icon": styles.widthIcon || SIDEBAR_WIDTH_ICON,
          } as React.CSSProperties
        }
      />
      <div
...
const SidebarTrigger = React.forwardRef<React.ElementRef<typeof Button>, Omit<React.ComponentProps<typeof Button>, "sidebarKey"> & { sidebarKey: string; icon?: React.ReactNode }>(({ className, onClick, sidebarKey, icon, ...props }, ref) => {
  const { toggleSidebar } = useSidebar(sidebarKey);
...
const SidebarRail = React.forwardRef<HTMLButtonElement, React.ComponentProps<"button"> & { sidebarKey: string }>(({ className, sidebarKey, ...props }, ref) => {
  const { toggleSidebar } = useSidebar(sidebarKey);
...
>(({ asChild = false, isActive = false, variant = "default", size = "default", tooltip, sidebarKey, className, ...props }, ref) => {
  const Comp = asChild ? Slot : "button";
  const { isMobile, state } = useSidebar(sidebarKey);
